...
public class WorkEvaluationService {
	...
	// 1. 관리자: 부서별 평가 내역 조회
	public List<WorkEvaluationDto.Response> getEvaluationsByDepartment(Long departmentId, int year, int quarter, Long employeeId) {
		...
		if (manager.getPosition() == null || manager.getPosition().getPositionLevel() == null || manager.getPosition().getPositionLevel() < 3
			|| !manager.getDepartment().getId().equals(departmentId)) {
			throw new IllegalAccessError("해당 부서의 관리자만 조회할 수 있습니다.");
		}

		// 2) 해당 부서의 평가 내역 반환
		List<WorkEvaluation> evaluations = workEvaluationRepository.findByEmployee_Department_IdAndEvaluationYearAndEvaluationQuarter(departmentId, year, quarter);
	...
	public WorkEvaluationDto.Response saveOrSubmitEvaluation(Long employeeId, WorkEvaluationDto.UpdateRequest request, int year, int quarter) {
		...
		Optional<WorkEvaluation> existing = workEvaluationRepository.findByEmployee_IdAndEvaluationYearAndEvaluationQuarter(employeeId, year, quarter);
